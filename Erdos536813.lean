...
/-!
### Exact states: `T = 80, 81`

Both states use the same upper board `D(16)`.  As above, the one-layer
bounds come from the structural axis-projection theorem, while `native_decide`
checks only the genuinely two-layer finite compatibility certificate.
-/
...
theorem twoLayer_80_deficit_two_bool :
    TwoLayerDeficitTwoCertificate
...
theorem twoLayer_81_deficit_two_bool :
    TwoLayerDeficitTwoCertificate
