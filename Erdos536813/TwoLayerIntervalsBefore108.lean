...
theorem selected_twoLayer_finite_range_before108_deficit_two
    ...
    (hLowerSelected : Erdos536.FiberSelectedComplete Lower A q)
    (hUpperSelected : Erdos536.FiberSelectedComplete Upper A (5 * q))
    (hLowerSub : Lower.Subset (Erdos536.FiberRegionList 1 T))
    (hUpperSub : Upper.Subset (Erdos536.FiberRegionList 1 (T / 5))) :
  ...
  rcases finiteTwoLayer_subsets_transfer_to_representative
      hFull hLowerSub hUpperSub with ⟨hLowerRep, hUpperRep⟩

  have hBenchmark :
      TwoLayerBenchmark T = TwoLayerBenchmark (FiniteTwoLayerRepresentative T) :=
  ...
  · have hRep : FiniteTwoLayerRepresentative T = 45 := by
    ...
    have hUpper : Upper.Subset (Erdos536.FiberRegionList 1 9) := by
      simpa using hUpperRep
    have hs : Lower.length + Upper.length + 2 ≤ 15 :=
      selected_twoLayer_45_deficit_two
        hq hA hLowerSelected hUpperSelected hLowerRep hUpper
    have hb : TwoLayerBenchmark 45 = 15 := by native_decide
    calc
      Lower.length + Upper.length + 2 ≤ 15 := hs
  ...
  · by_cases h54 : T < 54
    · have hRep : FiniteTwoLayerRepresentative T = 48 := by
      ...
      have hs : Lower.length + Upper.length + 2 ≤ 15 :=
        selected_twoLayer_48_deficit_two
          hq hA hLowerSelected hUpperSelected hLowerRep hUpper
      have hb : TwoLayerBenchmark 48 = 15 := by native_decide
      calc
        Lower.length + Upper.length + 2 ≤ 15 := hs
    ...
    · by_cases h60 : T < 60
      · have hRep : FiniteTwoLayerRepresentative T = 54 := by
        ...
        rw [hRep] at hLowerRep hUpperRep hBenchmark
        have hUpper : Upper.Subset (Erdos536.FiberRegionList 1 10) := by
          simpa using hUpperRep
        have hs : Lower.length + Upper.length + 2 ≤ 15 :=
          selected_twoLayer_54_deficit_two
            hq hA hLowerSelected hUpperSelected hLowerRep hUpper
        have hb : TwoLayerBenchmark 54 = 15 := by native_decide
        calc
          Lower.length + Upper.length + 2 ≤ 15 := hs
      ...
      · by_cases h64 : T < 64
        · have hRep : FiniteTwoLayerRepresentative T = 60 := by
          ...
          rw [hRep] at hLowerRep hUpperRep hBenchmark
          have hUpper : Upper.Subset (Erdos536.FiberRegionList 1 12) := by
            simpa using hUpperRep
          have hs : Lower.length + Upper.length + 2 ≤ 15 :=
            selected_twoLayer_60_deficit_two
              hq hA hLowerSelected hUpperSelected hLowerRep hUpper
          have hb : TwoLayerBenchmark 60 = 15 := by native_decide
          calc
            Lower.length + Upper.length + 2 ≤ 15 := hs
        ...
        · by_cases h72 : T < 72
          · have hRep : FiniteTwoLayerRepresentative T = 64 := by
              simp [FiniteTwoLayerRepresentative, h48, h54, h60, h64, h72]
            rw [hRep] at hLowerRep hUpperRep hBenchmark
            have hUpper : Upper.Subset (Erdos536.FiberRegionList 1 12) := by
              simpa using hUpperRep
            have hs : Lower.length + Upper.length + 2 ≤ 16 :=
              selected_twoLayer_64_deficit_two
                hq hA hLowerSelected hUpperSelected hLowerRep hUpper
            have hb : TwoLayerBenchmark 64 = 16 := by native_decide
            calc
              Lower.length + Upper.length + 2 ≤ 16 := hs
              _ = TwoLayerBenchmark 64 := hb.symm
              _ = TwoLayerBenchmark T := hBenchmark.symm

          · by_cases h81 : T < 81
            · have hRep : FiniteTwoLayerRepresentative T = 80 := by
                simp [FiniteTwoLayerRepresentative, h48, h54, h60, h64, h72, h81]
              rw [hRep] at hLowerRep hUpperRep hBenchmark
              have hUpper : Upper.Subset (Erdos536.FiberRegionList 1 16) := by
                simpa using hUpperRep
              have hs : Lower.length + Upper.length + 2 ≤ 17 :=
                selected_twoLayer_80_deficit_two
                  hq hA hLowerSelected hUpperSelected hLowerRep hUpper
              have hb : TwoLayerBenchmark 80 = 17 := by native_decide
              calc
                Lower.length + Upper.length + 2 ≤ 17 := hs
            ...
            · by_cases h90 : T < 90
              · have hRep : FiniteTwoLayerRepresentative T = 81 := by
                ...
                have hUpper : Upper.Subset (Erdos536.FiberRegionList 1 16) := by
                  simpa using hUpperRep
                have hs : Lower.length + Upper.length + 2 ≤ 18 :=
                  selected_twoLayer_81_deficit_two
                    hq hA hLowerSelected hUpperSelected hLowerRep hUpper
                have hb : TwoLayerBenchmark 81 = 18 := by native_decide
                calc
                  Lower.length + Upper.length + 2 ≤ 18 := hs
              ...
              · by_cases h96 : T < 96
                · have hRep : FiniteTwoLayerRepresentative T = 90 := by
                  ...
                  rw [hRep] at hLowerRep hUpperRep hBenchmark
                  have hUpper : Upper.Subset (Erdos536.FiberRegionList 1 18) := by
                    simpa using hUpperRep
                  have hs : Lower.length + Upper.length + 2 ≤ 18 :=
                    selected_twoLayer_90_deficit_two
                      hq hA hLowerSelected hUpperSelected hLowerRep hUpper
                  have hb : TwoLayerBenchmark 90 = 18 := by native_decide
                  calc
                    Lower.length + Upper.length + 2 ≤ 18 := hs
                ...
                · have h108 : T < 108 := by
                  ...
                  rw [hRep] at hLowerRep hUpperRep hBenchmark
                  have hUpper : Upper.Subset (Erdos536.FiberRegionList 1 19) := by
                    simpa using hUpperRep
                  have hs : Lower.length + Upper.length + 2 ≤ 18 :=
                    selected_twoLayer_96_deficit_two
                      hq hA hLowerSelected hUpperSelected hLowerRep hUpper
                  have hb : TwoLayerBenchmark 96 = 18 := by native_decide
                  calc
                    Lower.length + Upper.length + 2 ≤ 18 := hs
